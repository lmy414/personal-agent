...
function parseSessions() {
  ...
  console.log(`[sessions] homeDir=${homeDir} cwdKey=${cwdKey} sessionDir=${sessionDir} exists=${existsSync(sessionDir)}`);
  ...
  for (const file of files) {
    try {
      ...
      sessions.push({
        ...
        lastTimestamp: lastTimestamp || header.timestamp || new Date().toISOString(),
        name,
...
app.get("/api/sessions", (_req, res) => {
  try {
    ...
    console.log(`[sessions] API returning ${sessions.length} sessions, currentFile=${currentSessionFile}`);
    res.json({ sessions, currentFile: currentSessionFile });
  } catch (e) {
    console.error(`[sessions] API error: ${e.message}`);
  ...
  try {
    ...
    if (sid && typeof sid === "string" && sid.length > 0) {
      ...
      if (existsSync(sessionFile)) {
        ...
        console.log("[srv:observe] RETURN(session) turn=" + trace.turnIndex + " steps=" + (trace.steps||[]).length + " sessionId=" + (trace.sessionId||"").slice(-12) + " size=" + raw.length);
        return res.json(trace);
      }
    }
    // Fallback: serve live file
    const liveFile = join(paDir, "observe_last_trace.json");
    console.log("[srv:observe] GET fallback liveFile exists=" + existsSync(liveFile));
    if (existsSync(liveFile)) {
      const raw = readFileSync(liveFile, "utf-8");
      const trace = JSON.parse(raw);
      console.log("[srv:observe] RETURN(live) turn=" + trace.turnIndex + " steps=" + (trace.steps||[]).length + " sessionId=" + (trace.sessionId||"").slice(-12) + " size=" + raw.length);
      return res.json(trace);
    }
    console.log("[srv:observe] EMPTY — no trace found");
    return res.json({ turnIndex: 0, steps: [], timestamp: Date.now() });
  } catch(e) {
    console.error("[srv:observe] ERROR — " + e.message);
    res.status(500).json({ error: e.message });
  }
});

app.post("/api/observe_trace", (req, res) => {
  // No-op: pa-observe extension writes observe_last_trace.json directly.
  // Previously used for per-session trace files; now simplified.
  res.json({ ok: true });
});
